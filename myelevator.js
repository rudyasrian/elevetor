...
function check_request(a, requests, elevators) {
...
Elevator.prototype.decide = function() {
            ...
            if (elevator.direction == 'up') {
                ...
                for (a = elevator.at_floor(); a <= num_floors; a++) {
                    if (people.length < Config.elevator_capacity) {
                        if (check_request(a, requests, elevators)) {
    ...
    if (people == 0) {
        for (var i = 0; i < requests.length; i++) {
            ...
            if (!handled) {
                elevator.direction = elevator.at_floor() < elevator.get_destination_floor() ? 'up' : 'down';
